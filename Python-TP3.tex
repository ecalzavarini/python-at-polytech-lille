...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{Python-TP3_files/Python-TP3_7_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\item
  ...
  \(u_S\) et \(u_A\).
\item
  Enfin, il trace le graphique de la somme de \(u_S\) et \(u_A\).
\end{enumerate}
...
\item
  ...
  \(\Delta t = 0.25 \ s\), et on posera la valueur \(c = 1 \ m /s\) pour
  la vitesse de la perturbation dans le milieu.
\item
...
\(I_{O}(S)\) pour une plaque rectangulaire homogène en aluminium.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}12}]:} \PY{k+kn}{from} \PY{n+nn}{IPython.display} \PY{k+kn}{import} \PY{n}{Image}
         \PY{n}{Image}\PY{p}{(}\PY{n}{filename}\PY{o}{=}\PY{l+s}{\PYZsq{}}\PY{l+s}{fig\PYZus{}plaque.jpg}\PY{l+s}{\PYZsq{}}\PY{p}{)}
\end{Verbatim}
\texttt{\color{outcolor}Out[{\color{outcolor}12}]:}
    
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{Python-TP3_files/Python-TP3_43_0.jpg}
    \end{center}
    { \hspace*{\fill} \\}
